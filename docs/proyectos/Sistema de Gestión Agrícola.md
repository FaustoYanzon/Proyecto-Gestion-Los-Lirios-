...
1. **Trazabilidad — Fase 4: reemplazar los textos/fotos placeholder** de `frontend/lib/content/landing.ts` por contenido definitivo — a completar junto con los socios. Detalle: [[2026-09-03-trazabilidad-fase-3-4]].
2. **Chatbot de WhatsApp — chip activado en el celular, pendiente activación de línea con el proveedor.** Número: `2645263309`. Fausto lo insertó pero todavía no puede activarlo (probable registro de línea prepaga a DNI, exigencia ENACOM). Es un paso necesario: Meta verifica el número por SMS/llamada en el Paso 2 de Meta for Developers ("Register your WhatsApp phone number"), y sin la línea activa ese código nunca llega. Importante: no verificar ese número primero en la app común de WhatsApp (complica la migración al Business Platform) — dejarlo limpio para el registro directo en Meta. Una vez activada la línea: Paso 2 en Meta for Developers, actualizar `WHATSAPP_PHONE_NUMBER_ID`, reabrir `ngrok`.
...
5. **Misión, Visión y Valores** en Documentación > Empresa quedaron como placeholder "(a definir)" — Fausto lo va a terminar de definir con los socios.
