...
| Insumos / stock / plan fitosanitario / cumplimiento | ✅ | ✅ | parcial (solo carga de Fitosanitario) |
| Órdenes de aplicación fitosanitaria | ✅ | ✅ | ✅ (falta alta de usuarios operarios) |
...
## Próximos pasos (actualizado 2026-09-19)

Pilot estable. Trazabilidad **completa (Fase 0 a 4)** en producción: ficha por parcela + carta PDF + link público/QR sin login + landing institucional en `/`. Sistema de **fertilizantes/fitosanitarios con stock también completo (Fase 1 a 3)**: catálogo de Insumos con descuento automático, plan de aplicación por temporada cargado a mano, y cumplimiento plan-vs-real + necesidad de compra. **Órdenes de aplicación fitosanitaria en producción desde el 2026-09-19**: el plan (o una necesidad extra) genera órdenes ejecutables que cada operario confirma desde su propio celular — reemplaza el circuito verbal ingeniero→encargado→operario. Primer build de iOS ya en TestFlight (interno); Android con los íconos nuevos en revisión de Play Store; faltan los últimos pasos administrativos de ambas tiendas.

### Pendiente real

1. **Órdenes de aplicación — dar de alta las cuentas `User` de los operarios existentes en `Trabajador`.** Bloqueante real: sin login propio (`trabajador_id` vinculado), ningún operario puede confirmar una orden desde mobile. Detalle: [[2026-09-17-ordenes-aplicacion-fitosanitaria]].
2. **Inventarios — "Producto Terminado" queda en blanco a propósito.** Sección nueva creada en la barra lateral (`/dashboard/inventarios`, con Insumos ya andando ahí) pero la pestaña de producto terminado (uva/pasa/mosto cosechado) es un placeholder — Fausto quiere pensar primero cómo lo quiere llevar (¿por lote de cosecha? ¿con salidas por venta?) antes de diseñarlo.
3. **Trazabilidad — Fase 4: reemplazar los textos/fotos placeholder** de `frontend/lib/content/landing.ts` por contenido definitivo — a completar junto con los socios. Detalle: [[2026-09-03-trazabilidad-fase-3-4]].
4. **Chatbot de WhatsApp — chip activado en el celular, pendiente activación de línea con el proveedor.** Número: `2645263309`. Fausto lo insertó pero todavía no puede activarlo (probable registro de línea prepaga a DNI, exigencia ENACOM). Es un paso necesario: Meta verifica el número por SMS/llamada en el Paso 2 de Meta for Developers ("Register your WhatsApp phone number"), y sin la línea activa ese código nunca llega. Importante: no verificar ese número primero en la app común de WhatsApp (complica la migración al Business Platform) — dejarlo limpio para el registro directo en Meta. Una vez activada la línea: Paso 2 en Meta for Developers, actualizar `WHATSAPP_PHONE_NUMBER_ID`, reabrir `ngrok`.
5. **Play Store — confirmar aceptación real de los 12 verificadores, no solo que estén en la lista.** El panel oficial de Google (Panel de la app → "Producción") distingue "en la lista" de "aceptó participar" — al 2026-09-01 los 12 emails estaban cargados pero 0 habían aceptado. Cada uno tiene que abrir `https://play.google.com/apps/testing/com.loslirios.app` desde el celular (logueado con esa cuenta exacta de Gmail) y tocar "Convertirme en probador". Recién ahí arrancan los 14 días corridos exigidos antes de poder pedir "Acceso a producción". Sigue pendiente, sin novedades al 2026-09-07. Detalle: [[2026-09-01-ios-primer-build-play-store-14-dias]].
6. **iOS — sumar testers reales cuando haga falta.** Ninguno de los 12 verificadores de Android tiene cuenta de Apple todavía. Cuando se necesite, armar un grupo de **pruebas externas** en TestFlight (requiere Beta App Review de Apple la primera vez, a diferencia del grupo interno que ya tiene a Fausto probando). No hay mínimo de testers ni período de espera exigido por Apple, a diferencia de Google. Sigue pendiente, sin novedades al 2026-09-07.
7. **Misión, Visión y Valores** en Documentación > Empresa quedaron como placeholder "(a definir)" — Fausto lo va a terminar de definir con los socios.

**Resuelto desde la última actualización (no repetir):**
- **Órdenes de aplicación fitosanitaria — en producción el 2026-09-19.** El plan (o una necesidad extra fuera de plan) genera órdenes ejecutables con producto/dosis/parcela ya fijados; cada operario confirma desde su propio usuario en mobile qué parral aplicó (cantidad fija, solo observaciones/fotos editables) — reemplaza el circuito verbal ingeniero→encargado→operario. La confirmación genera un `RegistroFitosanitario` real (no una tabla paralela), así que la trazabilidad por parcela y el cumplimiento del plan ya la ven sin desarrollo extra. Backend+web+mobile desplegados y verificados en producción. Detalle: [[2026-09-17-ordenes-aplicacion-fitosanitaria]].
- **Fertilizantes y stock de Fitosanitarios — sistema completo (Fase 1 a 3), en producción el 2026-09-08.** Catálogo de `Insumo` con stock (descuento/reversión automática al cargar/editar/borrar un Fitosanitario real), `PlanFitosanitario` por temporada cargado a mano variedad por variedad (sin agrupar en "Tintas", sin importar el Excel del agrónomo), y cumplimiento plan-vs-real + necesidad de compra por insumo (matching automático, sin vínculo manual). Sección nueva "Inventarios" en la barra lateral. Detalle: [[2026-09-08-fertilizantes-stock-fitosanitarios]].
- **Catálogo de `Trabajador` con 14 nombres duplicados + combobox que los creaba en silencio** — resuelto el 2026-09-07. 52→31 trabajadores fusionados (backup previo), historial de tareas/riego/fitosanitarios re-apuntado, 14 registros huérfanos vinculados retroactivamente. El combobox de los 4 formularios (Tareas/Riego/Iniciar riego/Fitosanitarios, web y mobile) ya no acepta texto libre: solo elegir de la lista, o "+ Agregar nuevo trabajador" con confirmación (aviso de nombres parecidos). Defensa de servidor: 409 ante nombre duplicado. Detalle: [[2026-09-07-normalizacion-trabajadores-combobox]].
- **Nombres de los 4 usuarios nuevos del 2026-09-01** ("El Cauquen SRL", "Leticia Yanzon", "Mari Barcelo", "Fausto Yanzon") — confirmados correctos por Fausto el 2026-09-07, no hace falta corregir nada.
- **Catálogo de válvulas reales en mobile** (pendiente desde el 08-21) — confirmado por Fausto el 2026-09-07 que funciona perfecto.
- **Trazabilidad — Fase 4, fade del scroll de la landing** — confirmado por Fausto el 2026-09-07: "increíble", sin problemas.
...
### Hecho en la sesión del 2026-09-17 (órdenes de aplicación fitosanitaria, deploy 2026-09-19)

Ver [[2026-09-17-ordenes-aplicacion-fitosanitaria]] para el detalle completo. Pedido inicial de Fausto: la operatoria de fitosanitarios era 100% verbal (ingeniero → encargado → operarios) y abierta a errores de producto/dosis/parcela. Diseñado en conjunto antes de programar (Fausto confirmó: ambos alcances de parcela según el caso, confirmación con usuario propio de cada operario, cantidad fija al confirmar) y construido en una sola sesión, backend+web+mobile juntos.

- **La confirmación no duplica nada** — genera un `RegistroFitosanitario` real (mismo modelo del 09-08), vinculado vía `OrdenAplicacionParcela.registro_fitosanitario_id`. Reusa el descuento de stock y el cálculo de carencia/reingreso ya existentes; la trazabilidad por parcela y el cumplimiento del plan ven las aplicaciones confirmadas por orden automáticamente, sin ningún desarrollo extra.
- **Pool abierto, sin asignación nominal** — cualquier operario activo de la finca ve y confirma cualquier orden pendiente de su finca, no hay "orden asignada a Juan".
- **`User.trabajador_id` nuevo** — los operarios de campo no tenían login propio (solo una fila en el catálogo `Trabajador`). Se agregó el vínculo para resolver el responsable al confirmar sin pedírselo. **Dar de alta las cuentas reales sigue pendiente** — bloqueante para que la feature se use en la práctica.
- Backend: router `ordenes_aplicacion.py` (crear desde plan/extra, `GET /pendientes` con scope por finca, confirmar, subir foto a Cloudinary). Mobile: sección "Órdenes pendientes" en la tab Fito. Web: `/dashboard/produccion/ordenes-aplicacion` (generar + seguimiento con barra de progreso).
- **138/138 tests backend**, `tsc`/`eslint` limpios en mobile y frontend, verificado en vivo contra Postgres local antes de deployar. Un incidente real de entorno Windows (proceso zombie en el puerto 8000) se esquivó corriendo la verificación en el puerto 8001 — no era un problema de la feature.
- **Deploy (2026-09-19):** Railway auto (push `b5a2ff5`, verificado con `401` en vez de `404` contra el endpoint nuevo en producción), `vercel --prod`, `eas update --branch production --environment production` (100% JS, bundle verificado sin IP LAN).

### Hecho en la sesión del 2026-09-08 (fertilizantes, stock de insumos, plan y cumplimiento fitosanitario)

Ver [[2026-09-08-fertilizantes-stock-fitosanitarios]] para el detalle completo. Pedido inicial de Fausto: "empecemos a desarrollar la parte de fertilizantes y gestión de stock". Arrancó analizando el Excel real del agrónomo (`Programa aplicacion fitosanitario.xlsx`) y terminó en 3 fases, todas en producción:

- **Fase 1 — Insumos + stock.** Catálogo `Insumo` (nombre, unidad kg/lt, categoría, `stock_actual`) + ledger `MovimientoStock`. `RegistroFitosanitario` gana `insumo_id`/`unidad`/`cantidad_total` (campo viejo `dosis_lt_ha` renombrado a `dosis_por_ha` sin perder histórico, vía `alter_column` no drop+add) — al cargar una aplicación real, `cantidad_total = dosis_por_ha × superficie_ha de la parcela` se descuenta del stock, con reversión limpia si se edita o borra. Combobox nuevo (`InsumoSelect` web / `InsumoPicker` mobile) con el mismo criterio "elegí de la lista o agregá con doble verificación" que ya se usa para Trabajador — reuso directo, cero diseño nuevo de UX. Pantalla nueva de administración de Insumos.
- **Sección "Inventarios" en la barra lateral** (pedido aparte de Fausto, después de ver la Fase 1 andando): saca Insumos de Documentación y lo pone en su propia sección junto a "Producto Terminado" (placeholder en blanco a propósito).
- **Fase 2 — Plan Fitosanitario, rediseñado por Fausto.** Mi boceto original agrupaba en "Tintas" (como el Excel) e iba a importar el Excel automáticamente. Fausto lo cambió: **sin importación de Excel, carga a mano temporada tras temporada** (mismo espíritu que Metas de Producción, sincronizado con el selector global de campaña) y **sin agrupar en Tintas — variedad real por variedad**, porque esa agrupación es solo cómo el agrónomo lo comunica en su papel, no cómo tiene que vivir el dato. Al cargar una fila se puede marcar a qué otras variedades también aplica (se tipea una vez, se crean filas independientes, editables por separado después) — convenio para no repetir tipeo sin perder el control real por variedad. Solo gerencial+ carga/edita, el resto de los roles lo lee como referencia.
- **Fase 3 — Cumplimiento + necesidad de insumos vs. stock.** Decisión explícita de Fausto: el vínculo entre una aplicación real y una fila del plan **no se carga a mano** (no tocar el formulario de Fitosanitarios, ya validado en producción) — se calcula cruzando variedad + insumo + fecha dentro de la campaña. Cuando el mismo producto tiene varias rondas planificadas, cada aplicación real cubre la siguiente ronda por **orden cronológico** (heurística razonable, no infalible si las rondas se aplican fuera de orden). Página nueva con avance por variedad (barra de % + estado) y una tabla de necesidad de insumo vs. stock con alerta de faltante — reemplaza directamente la tabla rota (`#REF!`) del Excel del agrónomo.
- **129/129 tests backend**, verificado de punta a punta en el navegador local con datos reales en cada fase (incluido el ciclo completo: cargar aplicación real → el cumplimiento sube → la necesidad de insumo baja, en tiempo real). Deploy: Railway (con 2 migraciones, la de Fase 1 y la de Fase 2; Fase 3 no tocó el esquema) + `vercel --prod` en cada fase — esta vez ninguno de los deploys fue bloqueado por el classifier de Claude Code, a diferencia de sesiones anteriores. Mobile: `eas update` solo en la Fase 1 (única que tocó el wizard de carga de Fitosanitarios); Fases 2/3 son pantallas de planificación/gerencial, no viven en mobile.
- **Hallazgo operativo de esta sesión, no de la app:** procesos `uvicorn --reload` huérfanos en Windows (el proceso padre "reloader" muere pero el hijo worker queda vivo, bindeado al mismo puerto) causaron respuestas intermitentes con código viejo durante la verificación local de la Fase 3 — nada que ver con el código de la feature, ver [[feedback_local_testing_before_deploy]] si se repite.

### Hecho en la sesión del 2026-09-07 (normalización de Trabajador + combobox "solo elegir de la lista")

Ver [[2026-09-07-normalizacion-trabajadores-combobox]] para el detalle completo. Fausto encontró trabajadores repetidos en el maestro de Documentación y pidió cerrar el problema de raíz, no solo prolijizarlo. Causa real: el combobox de Trabajador (desde el 08-05) creaba un registro nuevo en silencio cada vez que el nombre tipeado no matcheaba exacto — 14 nombres terminaron con 2-3 variantes (mayús/minús, con/sin tilde, repetidos), más 3 filas "ZZZ..." de pruebas viejas nunca borradas, más 14 registros de tareas/riego con `trabajador_id` NULL por typos/abreviaturas. **Datos:** `scripts/normalizar_trabajadores.py` (mismo patrón dry-run + `--commit` + backup que scripts previos) fusionó los 18 duplicados y vinculó los huérfanos — dos casos abreviados (`"Orlando M."`, `"Jesús V"`) resueltos por contexto de fecha/tarea, no adivinados. 52→31 trabajadores, 0 duplicados/huérfanos verificado después. **Combobox:** componente compartido nuevo en web (`TrabajadorSelect.tsx`, reemplaza `ResponsableInput.tsx` y el combobox inline de `TareaForm.tsx`) y en mobile (`TrabajadorPicker.tsx`, primera componentización de esto en esa plataforma) — usados en los 4 puntos de carga (Tareas/Riego/Iniciar riego/Fitosanitarios). Ya no se puede confirmar sin elegir de la lista; "+ Agregar nuevo trabajador" pide un segundo clic con aviso de nombres parecidos. Backend: `POST /trabajadores/` rechaza duplicados (case/tilde-insensitive) con 409. 111/111 tests backend, verificado en vivo en el navegador local (alta real, aviso de parecidos, defensa 409). Deploy: Railway auto (backend), `vercel --prod` por Fausto (el classifier de Claude Code lo bloqueó de forma consistente, no solo las escrituras a la DB), `eas update --branch production --environment production` (100% JS) verificado sin IP LAN.
