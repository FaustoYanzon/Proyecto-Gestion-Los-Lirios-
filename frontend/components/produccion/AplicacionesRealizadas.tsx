...
function exportCSV(data: FitosanitarioResponse[], parcelaNombre: (id: string) => string) {
  ...
  const rows = data.map((r) => [
    ...
    r.dias_carencia,
    r.dias_reingreso,
    r.fecha_habilitacion_cosecha,
    r.fecha_habilitacion_reingreso,
    r.responsable,
  ])
  ...
  a.href = url
  a.download = `fitosanitarios-${new Date().toISOString().split('T')[0]}.csv`
...
// Historial de aplicaciones realizadas -- pestaña "Aplicadas" de Órdenes de
// Aplicación. Ya no se cargan a mano: cada fila nace cuando un operario
// confirma una orden en mobile. Gerencial+ puede corregir o borrar una fila
// (mismo permiso que exige el backend).
export default function AplicacionesRealizadas() {
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
        <div className="flex flex-wrap items-end gap-3">
          ...
          {registros.length > 0 && (
            <button
              onClick={() => exportCSV(registros, parcelaNombre)}
              className="ml-auto flex items-center gap-2 px-3 py-2 text-sm font-medium text-gray-600 border border-gray-300 rounded-md hover:bg-gray-50 transition-colors"
